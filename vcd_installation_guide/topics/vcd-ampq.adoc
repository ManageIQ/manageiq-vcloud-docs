...
To enable AMPQ in the vCloudDirector a login in as system administrator is required. Note that it is not enough to have (only) administrator role inside the Organisation. After this navigate to AMPQ settings following "System -> Administration -> Management -> System settings -> Extensibility".
